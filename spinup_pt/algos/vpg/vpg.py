...
import spinup_pt.algos.vpg.core as core
from spinup_pt.utils.logx import EpochLogger
from spinup_pt.utils.mpi_tools import mpi_fork, proc_id, mpi_statistics_scalar, num_procs
...
"""
def vpg(env_fn, actor_critic=core.Actor_Critic, ac_kwargs=dict(), seed=0, 
        steps_per_epoch=4000, epochs=50, gamma=0.99, pi_lr=1e-3,
    ...
    def update():
        ...
        pi_l_old = pi_loss
        approx_ent = torch.mean(-logp)                  # a sample estimate for entropy, also easy to compute
        ...
        approx_kl = torch.mean(logp_old_ph - logp)      # a sample estimate for KL-divergence, easy to compute

        logger.store(LossPi=pi_l_old.item(), LossV=v_l_old.item(), 
                     KL=approx_kl.item(), Entropy=approx_ent.item(), 
...
if __name__ == '__main__':
    ...
    vpg(lambda : gym.make(args.env), actor_critic=core.Actor_Critic,
